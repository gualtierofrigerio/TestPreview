...
class DataSourceCommon {
    ...
    class func getTestUsers() -> [User] {
        guard let testDataPath = Bundle.main.url(forResource: "testdata", withExtension: "json"),
            let testData = try? Data(contentsOf: testDataPath),
            let users = DataSourceCommon.decodeData(data: testData, type: [User].self) as? [User] else {
            return []
        }
        return users
    }
